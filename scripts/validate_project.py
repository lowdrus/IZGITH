...
def main():
    ...
    pkg=read_json(ROOT/'package.json') or {};mv=str(m.get('version',''));pv=str(pkg.get('version',''));mm=re.fullmatch(r'(\d+\.\d+\.\d+)\.(\d+)',mv);expected=f'{mm.group(1)}-{int(mm.group(2)):05d}' if mm else '';expected_registry=f'{mm.group(1)}.{int(mm.group(2)):05d}' if mm else '';check('versões sincronizadas',bool(mm) and pv==expected);check('versão registry sincronizada',registry.get('version')==expected_registry)

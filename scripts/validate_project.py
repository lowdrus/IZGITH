...
def main():
    ...
    conv=EXT/'integrations/conv-d.js';ct=conv.read_text(encoding='utf-8') if conv.is_file() else ''
    required=['izgith.conv-d.v3','ChatGPT','Claude','Gemini','pdf','doc','txt','md','json','xls','Baixar Conversa','Tudo — início até o final','Rodada','SAVE_FILE','saveAs:true']
    check('CONV-D export contract',all(x in ct for x in required))

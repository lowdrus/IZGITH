...
def main():
    ...
    registry=read_json(ROOT/'integrations/assistant_registry.json');names=[x.get('name') for x in registry.get('canonical_assistants',[]) if isinstance(x,dict)] if isinstance(registry,dict) else [];check('IA/assistentes',names==['Júlia','Ayella','IZART'] and not any(x in str(names) for x in ('Ayelle','alias Ayella','Alias: Ayella')))
    check('service worker sem alias proibido',not any(x in wt for x in ('Ayelle','alias Ayella','Alias: Ayella')));check('registry IA canônico',names==['Júlia','Ayella','IZART'] and registry.get('naming_policy',{}).get('ayella_is_alias') is False);check('registry sem aliases proibidos',not any(x in names for x in ('Ayelle','alias Ayella','Alias: Ayella')))

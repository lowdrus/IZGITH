...
$('btnDownload').addEventListener('click', async () => {
  ...
  try {
    const id = await chrome.downloads.download({ url: url.href, saveAs: true });
...
$('packages').addEventListener('change', (event) => addFiles(event.target.files));

const dz = $('dropzone');
for (const eventName of ['dragenter', 'dragover']) {
  dz.addEventListener(eventName, (event) => { event.preventDefault(); dz.classList.add('active'); });
}
for (const eventName of ['dragleave', 'drop']) {
  dz.addEventListener(eventName, (event) => { event.preventDefault(); dz.classList.remove('active'); });
}
dz.addEventListener('drop', (event) => addFiles(event.dataTransfer.files));

dz.addEventListener('keydown', (event) => {
  if (event.key === 'Enter' || event.key === ' ') $('packages').click();
});
...
$('btnGuide').addEventListener('click', async () => {
  await chrome.storage.local.set({ izgithQueue: state.files.map(({ name, size, lastModified }) => ({ name, size, lastModified })) });
  chrome.tabs.create({ url: chrome.runtime.getURL('dashboard.html#install') });
});

...
1. Trabalhar somente na extensão distribuível em `extension/`, no host em `host/`, nos testes, scripts e documentação correspondentes.
...
4. Antes de cada commit, executar `npm test`, `npm run package`, `python -m compileall -q host scripts tools tests` e `git diff --check`.

...
function Get-FileSafe([string]$Url,[string]$Path) { try { [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12 } catch {} try { $wc=New-Object Net.WebClient; $wc.Headers['User-Agent']='IZGITH-Builder/00050'; $wc.DownloadFile($Url,$Path) } catch { if(Test-Path -LiteralPath $Path){Remove-Item -LiteralPath $Path -Force -ErrorAction SilentlyContinue}; throw } }
function Expand-ZipCompat([string]$Zip,[string]$Dest) { Add-Type -AssemblyName System.IO.Compression.FileSystem -ErrorAction SilentlyContinue; if([Type]::GetType('System.IO.Compression.ZipFile, System.IO.Compression.FileSystem')){[IO.Compression.ZipFile]::ExtractToDirectory($Zip,$Dest);return}; $shell=New-Object -ComObject Shell.Application; $src=$shell.NameSpace($Zip); $dst=$shell.NameSpace($Dest); if($null -eq $src -or $null -eq $dst){throw 'ZIP extraction is unavailable on this Windows installation.'}; $dst.CopyHere($src.Items(),20) }
Write-Host 'IZGITH 00050 - FULL BUILD' -ForegroundColor Cyan
Write-Host ('Target: ' + $Target)
if(Test-Path -LiteralPath $Target){Remove-Item -LiteralPath $Target -Recurse -Force}
New-Item -ItemType Directory -Path $Target -Force | Out-Null
$sourceRoot=Join-Path $Target '_source'; New-Item -ItemType Directory -Path $sourceRoot -Force | Out-Null
...
Expand-ZipCompat $repoZip $sourceRoot
$top=Get-ChildItem -LiteralPath $sourceRoot -Directory | Select-Object -First 1
...
foreach($d in @('host','scripts','tools','docs','integrations','build')){$src=Join-Path $top.FullName $d;if(Test-Path -LiteralPath $src -PathType Container){Copy-Item -LiteralPath $src -Destination (Join-Path $Target $d) -Recurse -Force}}
...
@('@echo off','setlocal','powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0IZGITH_BUILD_00050.ps1"','exit /b %ERRORLEVEL%') | Set-Content -LiteralPath (Join-Path $Target 'IZGITH_BUILD_00050.bat') -Encoding ASCII
Write-Host '[5/5] Structural validation...' -ForegroundColor Yellow
$manifest=Join-Path $Target 'extension\manifest.json';$sw=Join-Path $Target 'extension\sw.js'
...
$manifestText=[IO.File]::ReadAllText($manifest);if($manifestText -notmatch '"manifest_version"\s*:\s*3'){throw 'Manifest is not MV3.'};if($manifestText -notmatch '"service_worker"\s*:\s*"sw\.js"'){throw 'Manifest service_worker is not sw.js.'}
$swText=[IO.File]::ReadAllText($sw);if($swText -match '\btype\s*=\s*"'){throw 'Found invalid JavaScript type= initializer.'};if($swText -match '\?\.') {throw 'Optional chaining found in service worker.'}

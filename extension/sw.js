...
const NATIVE_HOST = 'com.izgith.proplus';
...
function nativeHostCheck(sendResponse) {
  ...
  try {
    ...
    sendResponse({ ok: false, available: false, error: String(error?.message || error) });
    return;
  }
  ...
  const timer = setTimeout(() => {
    finish({ ok: false, available: false, error: 'Native host did not respond.' });
  }, 2500);
  ...
  port.onDisconnect.addListener(() => {
    clearTimeout(timer);
    // Reading lastError prevents the classic
    // "Unchecked runtime.lastError" console warning.
    const runtimeError = chrome.runtime.lastError;
    finish({
      ok: false,
      available: false,
      error: runtimeError?.message || 'Native host disconnected without a response.'
    });
  });
  ...
  try {
    ...
    clearTimeout(timer);
    finish({ ok: false, available: false, error: String(error?.message || error) });
  }
}

/* Adaptado para IZGITH; fonte e licença em MYJDOWNLOADER. */
(() => {
'use strict';
...
// Sessões de publicação não são expostas aos content scripts.
 ...
 chrome.tabs.sendMessage(tabId, { action: "open-in-page-toolbar", tabId: tabId })
  .catch(() => {
   ...
   chrome.scripting.executeScript({
    target: { tabId: tabId },
    files: ['modules/jdownloader/contentscripts/toolbarContentscript.js']
   }).then(() => {
    setTimeout(() => {
...
// ============================================================
let offscreenDocumentPath = 'modules/jdownloader/offscreen.html';
...
function updateBadge() {
 ...
 let text = ""; // módulo opcional não altera a saúde global do IZGITH
 let color = "#f3d435";
...
const RELEASES_API = 'https://api.github.com/repos/lowdrus/IZGITH/releases/latest';
const RELEASES_PAGE = 'https://github.com/lowdrus/IZGITH/releases/latest';
...
function getBuildTimestamp() {
 if (!buildTimestampPromise) {
  buildTimestampPromise = (async () => {
   try {
    const resp = await fetch(chrome.runtime.getURL('modules/jdownloader/buildMeta.json'));
    if (!resp.ok) return 0;
...
async function initSettings() {
 ...
 settings[STORAGE_KEYS.CLICKNLOAD_ACTIVE] = result[STORAGE_KEYS.CLICKNLOAD_ACTIVE] ?? false;
 settings[STORAGE_KEYS.CONTEXT_MENU_SIMPLE] = result[STORAGE_KEYS.CONTEXT_MENU_SIMPLE] ?? false;
...
initSettings().catch(error => console.error("JDOWNLOADER:", error));
